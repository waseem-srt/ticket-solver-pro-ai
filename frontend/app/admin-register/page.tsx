...
export default function AdminRegisterPage() {
  const { adminRegister, loading, error, setError } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLocalError("");
    await adminRegister(email, password, inviteCode);
  };
    ...
    >
      ...
      <Card className="w-full max-w-md glass border-border/50 relative z-10">
        <CardHeader className="text-center pb-6">
          ...
          <CardDescription>Create an administrator account with invite code</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground/80">Invite Code</label>
              <div className="relative">
                <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input id="admin-invite-code" type="text" placeholder="Enter admin invite code" value={inviteCode}
                  onChange={(e) => { setInviteCode(e.target.value); setError(null); setLocalError(""); }}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="admin-reg-email" type="email" placeholder="admin@company.com" value={email}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="admin-reg-password" type="password" placeholder="Min. 8 characters" value={password}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input id="admin-reg-confirm" type="password" placeholder="Repeat password" value={confirm}
                  onChange={(e) => { setConfirm(e.target.value); setLocalError(""); }}
                  className="pl-10 bg-secondary/50 border-border/50" required />
              </div>
            </div>
            <Button id="admin-register-submit" type="submit" className="w-full h-11 font-semibold text-white hover:opacity-90 transition-opacity"
            ...
            <div className="space-y-2 text-center text-sm text-muted-foreground">
              <p>
                ...
                <Link href="/login" className="font-medium hover:underline" style={{ color: "oklch(0.68 0.23 25)" }}>
                  Admin sign in
                </Link>
              </p>
              <p>
                Not an admin?{" "}
                <Link href="/register" className="text-primary hover:underline font-medium">User sign up</Link>
              </p>
            </div>
          </form>
        </CardContent>
